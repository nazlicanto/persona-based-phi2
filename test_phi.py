...
def run_model(config):
    ...
    """
    model = AutoModelForCausalLM.from_pretrained("agents/phi2_model/microsoft--phi-2", trust_remote_code=True)
    ...
    """
    """
    model = AutoModelForCausalLM.from_pretrained("merged_adapters", trust_remote_code=True)
    model.push_to_hub("nazlicanto/phi-2-persona-chat", token="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")
    model.to("cuda")
    """
    model = AutoModelForCausalLM.from_pretrained(config.model_id, trust_remote_code=True)
    model.to("cuda")
